...
- 网站使用 `docs/source/_static/images/generated/` 中人工验收并提交的本地 PNG；
  GitHub Actions 运行示例和 Sphinx 构建，但默认不重新生成、覆盖这些网站图片。
- 图片生成脚本继续保留，供本地复现和修改；更新图片时应在本机生成、逐图目视检查后，
  将脚本与最终 PNG 一起提交。只有 CI 已明确安装并验证中文字体时，才考虑恢复自动绘图。
- 中文必须在生成 PNG 时正确栅格化，不能依赖浏览器或部署服务器补字体。
- 流程图方框按实际多行文字扩容，文字不得越界、互相覆盖或压住脚注；优先增加画布、
  行高和框体尺寸，不以过度缩小字号掩盖布局问题。
- xlabel、tick label 和 table 单元格不得直接显示 `np.float64(...)`、`np.int64(...)`
  等 NumPy 对象表示；先转换为 Python `float` / `int`，数值最多保留两位有效数字。
