...
are intentionally imported inside :func:`ppt2picture` so importing ``mymetal``
remains possible on systems without Microsoft PowerPoint.
"""
...
def _normalize_style(style: str) -> tuple[str, str, str]:
    ...
    if style_normalized not in SUPPORTED_STYLES:
        ...
        raise ValueError("Unsupported style: " + style + ". Choose from: " + supported)
...
def _get_background_color(image: Image.Image) -> tuple[int, int, int]:
    ...
    for box in lbox:
        lpixel.extend(image.crop(box).getdata())
...
def _crop_image_whitespace(
    ...
    dict_save_kwargs: dict[str, Any] = {"format": pillow_format, "dpi": (dpi, dpi)}
...
def ppt2picture(
    ...
    try:
        ...
        raise RuntimeError("Failed to export " + str(path_ppt) + ": " + str(error)) from error
    finally:
