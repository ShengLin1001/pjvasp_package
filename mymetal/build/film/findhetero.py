"""
findhetero submodule

This submodule provides functions for finding the heterostructure of two thin film structures.
Only used when you install the hetbuilder package.

Functions:
    - find_hetero: Finds a heterostructure using the hetbuilder API.
    - my_plot_results: Interactive visualization of the results via matplotlib.
    - set_length: Set the length of the atoms along the axis.
    - magnitude: Compute the magnitude of the vector.
    - build_supercells: Constructs supercells for two primitive cells using the transition matrix and rotation angle of the top layer around the z-axis.
    - stack_atoms: Stacks two supercells to form a heterostructure.
    - scale_cell_xy: Scales the x-y position of the cell while fixing the z-direction.
    - split_model: Splits a heterostructure into a substrate and film according to the atom type.
"""
