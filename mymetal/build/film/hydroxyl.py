...
This submodule provides functions for adding hydrogen atoms to passivate dangling bonds on the surface of a slab.

Functions:
    - add_hydro_atoms: Adds hydrogen atoms to the surface of the atoms object.
    - get_neighbors: Gets the list of neighbors for a given atom, along with their relative displacements and distances.
    - find_matching_atom_in_bulk: Finds the corresponding atom in the bulk based on the position of an atom in the slab.
    - find_unsaturated_neighbors: Identifies missing neighbors by comparing displacement vectors between bulk and slab neighbors.
    - passivate_surface_custom: Adds adsorbates (e.g., hydrogen) to passivate dangling bonds on the surface of a slab.
"""
