...
def generate_slurm_script_shared_parent(
        ...
        wall_time: str = None) -> str:
    """生成一个并发运行多个 each_subdir chunk worker 的父脚本。
    ...
    line_parent = (
        ...
        "\n"
        'echo ""\n'
        'echo "================ 📊 Summary (mode=each_subdir, parent=shared)"\n'
...
def check_chunk_parent_layout(
    ...
    ``auto`` 只在 each_subdir 下收拢为一个 shared 父作业；single_alloc 的每个
    chunk 必须持有自己的计算资源分配，因此始终解析为历史 ``per_chunk`` 布局。

    Args:
        chunk_parent_layout: ``auto``、``shared`` 或 ``per_chunk``。
        mode: 当前提交模式。
        lchunk_parent_layout: 允许的布局列表。

    Returns:
        解析后的 ``shared`` 或 ``per_chunk``。

    Raises:
        SystemExit: 当布局未知，或尝试为非 each_subdir 使用 shared 时退出。
    """
    ...
    if chunk_parent_layout == "shared" and mode != "each_subdir":
        fail("chunk_parent_layout=shared is only supported for each_subdir mode")
...
def pei_slurm_univ_submit(
                    ...
                    MODES: list = ["parallel", "each_subdir", "single_alloc"],
    ...
    Args:
        ...
        mode: 运行模式，可选 "parallel"、"each_subdir" 或 "single_alloc"。
        dir_root: path_root 下的递归搜索根；为空列表时发现其中所有 ``y_dir``。
        lsubdir: 可选的计算子目录 basename 过滤列表；为空时汇总所有 ``y_dir`` 的一级子目录。
        chunks: 在顺序编排模式下将子目录划分的组数。
        chunk_parent_layout: chunk worker 的父作业布局；auto 在 each_subdir 下使用
            shared，在 single_alloc 下保持 per_chunk。
        module_profile_type: 环境模块配置类型，对应 MODULE_BLOCKS 中的键。
